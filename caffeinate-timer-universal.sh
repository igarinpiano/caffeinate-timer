...
# 実行中のタイマー/待機ループでは終了フラグだけを立て、本体側で確実に後始末する。
_caffeinate_pid=""
...
# 存在しない場合は dirname + pwd -P + basename で手動解決する。
...
# curl を優先し、存在しない場合は wget にフォールバック（Linux 対応）。
# --proto '=https' で HTTPS のみ許可（HTTP へのダウングレード防止）。
# --max-redirs 5 でリダイレクトループを防止。
# curl / wget どちらも存在しない場合は失敗を返す。
# 取得内容が空の場合も失敗扱い。
_ct_fetch_versions() {
  local _content
...
# ── アップデート: ダウンロード・自己置換 ─────────────────
# 処理順: セキュアな一時ディレクトリ作成（700）→ DL（curl 優先・wget フォールバック）
...
_ct_download_replace() {
  ...
  if ! command -v curl &>/dev/null && ! command -v wget &>/dev/null; then
    printf '%s\n' "${RED}❌ curl も wget も見つかりません。どちらかをインストールしてください。${RESET}"
    printf '\n'
    read -r -p "Enterで閉じる..." _
    exit 1
  fi
  ...
  if [ "$_dl_ok" -eq 0 ]; then
    rm -rf "$_tmpdir"
    printf '%s\n' "${RED}❌ ダウンロードに失敗しました。（終了コード: ${_dl_exit}）${RESET}"
    printf '%s\n' "  ・ネットワーク接続を確認してください。"
  ...
  if [[ "$OS" == "Darwin" ]]; then
    ...
    _orig_perms=$(stat -c "%a" "$_CT_SCRIPT_PATH" 2>/dev/null) || _orig_perms="755"
  fi
...
_ct_auto_update() {
  ...
  _vc=$(_ct_fetch_versions) || {
    local _fetch_hint=""
    if ! command -v curl &>/dev/null && ! command -v wget &>/dev/null; then
      _fetch_hint=" curl または wget をインストールしてください。"
    fi
    printf '%s\n' "${RED}❌ バージョン情報の取得に失敗しました。ネットワーク接続を確認してください。${RESET}"
    [ -n "$_fetch_hint" ] && printf '%s\n' "  ${_fetch_hint}"
    printf '\n'
    read -r -p "Enterで閉じる..." _
    exit 1
  }
...
# y/mo: macOS は BSD date -v、Linux は GNU date -d によるカレンダー演算。
_ct_parse_adj_secs() {
  local _a="$1" _sign=1
...
# ── タイマー UI: 2行表示 helper ──────────────────────────
# カウントダウンループ中にステータス行と調整入力行を
# 2行固定で上書きし続けるための helper 群。
# _ct_timer_ui_begin  : 2行分の空行を確保し、表示領域を初期化する。
# _ct_timer_ui_render : ANSI エスケープでカーソルを戻し、2行を上書きする。
# _ct_timer_ui_clear  : ループ終了後に2行をクリアしてカーソルを左端に戻す。
...
_ct_timer_ui_render() {
  ...
  printf '\r\033[1A'
  printf '\033[2K%s\n' "$_status"
  printf '\033[2K  %s調整入力:%s %s' "$CYAN" "$RESET" "$_buffer"
}
...
if [[ "$input" == "/until "* ]]; then
  ...
  if [[ "$OS" == "Darwin" ]]; then
    ...
    # GNU date: "today HH:MM:SS" 形式でエポック秒を算出する。
    _target_epoch=$(date -d "$(date -d "@${_now_epoch}" +%Y-%m-%d) $(printf '%02d:%02d:00' "$_u_h" "$_u_m")" +%s) || {
      printf '%s\n' "${RED}❌ 時刻の計算に失敗しました。${RESET}"
      printf '\n'
      read -r -p "Enterで閉じる..." _
      exit 1
...
# -t / sleep 秒数 は使用しない（時間調整による延長に対応するため）。
