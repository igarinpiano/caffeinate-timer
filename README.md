...
プレゼンテーション中、大きなファイルのダウンロード中、動画の書き出し中など、「今だけは Mac にスリープしてほしくない」という場面で、指定した時間だけスリープを防止します。

macOS 標準の `caffeinate` コマンド（`-u` および `-d` フラグ）を、より直感的かつ簡単に使えるようにしたラッパーツールです。
