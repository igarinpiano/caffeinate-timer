...
`/wait <プロセス名またはPID>` と入力すると、指定したプロセスが終了するまでスリープを防止します。動画エンコードやダウンロードなど、完了タイミングが不明な作業に便利です（全プラットフォーム対応）。
...
`/bg <時間>` と入力すると、スリープ防止をバックグラウンドで実行しターミナルウィンドウを即座に閉じられます。終了時はデスクトップ通知でお知らせします（全プラットフォーム対応）。
...
### 緊急修正版（サフィックス付きバージョン）について

重大なバグやセキュリティ上の問題が見つかった場合、`v1.4.1a` のようにアルファベットのサフィックスを付けた緊急修正版をリリースすることがあります。

- 緊急修正版は `/update` による自動アップデートで通常版と同様に配信されます。
- サフィックス付きバージョンの修正内容は、次の通常版（数字のみ）に正式に取り込まれます。緊急修正版を導入済みのユーザーは、次の通常版リリース時に自動アップデートで自然に移行できます。
