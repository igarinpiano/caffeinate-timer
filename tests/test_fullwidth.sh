...
for SCRIPT in $CT_TARGETS; do
  ...
  for v in '+90' '-5' '＋90' '－5' '−5' 'ー5' '+1h' '-1h' '＋１ｈ' '－１ｈ' 'ー1h' \
           '+1:30' '－1:30' 'ー1:30' '+45m' '－45m' 'ー45m' '+1y' '－1y' 'ー1y'; do
  ...
  if ! printf '%s' "$FN" | grep -q '_ct_parse_adj_secs'; then
    ...
    assert_eq "$(adj '-1h')"  "$(adj 'ー1h')"    "'-1h' ⇔ 'ー1h'（U+30FC 長音符）"
    assert_eq "$(adj '+30m')" "$(adj '＋３０ｍ')" "'+30m' ⇔ '＋３０ｍ'（全て全角）"
    assert_eq "$(adj '-1h')"  "$(adj '－１ｈ')"  "'-1h' ⇔ '－１ｈ'（全て全角）"

    # 符号は先頭だけを対象にする（末尾や単独は拒否）
    assert_eq "$(adj '-30m')" "$(adj 'ー30m')" "'-30m' ⇔ 'ー30m'"
    assert_eq "" "$(adj '30mー')" "'30mー'（末尾の長音符）は拒否される"
    assert_eq "" "$(adj 'ー')"    "'ー' だけは拒否される"
